...
/// <summary>
/// An HTML to Markdown converter: tokenizer, sanitiser, and block writer.
/// </summary>
internal sealed class SubstackHtmlConverter
{
        ...
        IReadOnlyDictionary<int, HtmlImageResolution> imageResolutions)
    {
        ...
        var transformations = new List<ImportTransformation>();
    ...
    private static bool HandleSkip(
        HtmlToken token,
        Stack<string> skip,
        ICollection<ImportTransformation>? transformations,
    ...
    private static void AddTransformation(
        ICollection<ImportTransformation> result,
        ISet<string> seen,
        string kind,
        string detail,
        string sourcePointer)
    {
        if (seen.Add(kind))
        {
            result.Add(new ImportTransformation(kind, detail, sourcePointer, null));
        }
    }

    private static void AddWarning(
        ICollection<ImportWarning> result,
        ISet<string> seen,
        string code,
        string severity,
        string message,
        string sourcePointer)
    {
        ...
        if (seen.Add(key))
        {
            result.Add(new ImportWarning(code, severity, message, sourcePointer));
        }
    }
...
internal sealed record HtmlConversionResult(
    string Markdown,
    IReadOnlyList<ImportTransformation> Transformations,
